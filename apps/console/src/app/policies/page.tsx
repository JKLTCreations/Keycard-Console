...
const columns: Column<Policy>[] = [
  {
    key: "name",
    header: "Name",
    render: (p) => <span className="text-[13px] text-text-primary">{p.name}</span>,
  },
  {
    key: "version",
    header: "Version",
      ...
      return (
        <div className="flex items-center gap-2 text-[12px]">
          ...
          <span className="text-text-faint">
            ({allows > 0 && <span className="text-emerald-500">{allows}a</span>}
            {denies > 0 && <>{allows > 0 && "/"}<span className="text-red-500">{denies}d</span></>}
            {escalates > 0 && <>{(allows > 0 || denies > 0) && "/"}<span className="text-amber-500">{escalates}e</span></>})
          </span>
        </div>
      );
    },
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      <PageHeader
        ...
        action={
          <button className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-medium bg-text-primary text-surface-0 rounded-md hover:bg-text-secondary transition-colors duration-100 cursor-pointer">
