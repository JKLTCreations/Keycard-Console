...
export default function AgentDetailPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        {agent.status === "active" && (
          <button
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 text-[13px] font-medium rounded-md border border-[#ec5d5e20] text-[#ec5d5e] hover:bg-[#ec5d5e08] transition-colors"
          >
      ...
      {showConfirm && (
        <div className="mb-6 rounded-md border border-[#ec5d5e20] bg-[#ec5d5e05] p-4">
          <p className="text-[13px] text-[#ec5d5e] mb-3">
            Are you sure you want to revoke this agent? This action cannot be undone.
          </p>
          <div className="flex gap-2">
            <button onClick={handleRevoke} className="px-3 py-1.5 text-[13px] font-medium rounded-md bg-[#ec5d5e] text-white hover:opacity-90 transition-opacity">
              Confirm Revoke
            </button>
            <button onClick={() => setShowConfirm(false)} className="px-3 py-1.5 text-[13px] font-medium rounded-md border border-border text-text-secondary hover:bg-[#ffffff05] transition-colors">
              Cancel
            </button>
          </div>
        </div>
      ...
      <div className="rounded-lg border border-border bg-surface-1 p-5 mb-8">
        ...
        <pre className="text-[11px] text-text-secondary font-mono bg-surface-0 rounded-md border border-border-subtle p-4 overflow-x-auto">
