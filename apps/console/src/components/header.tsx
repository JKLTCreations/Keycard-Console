...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center h-12 px-6 border-b border-border-subtle bg-surface-0/80 backdrop-blur-sm">
      <nav className="flex items-center gap-1.5 text-sm">
        ...
        {breadcrumbs.map((crumb) => (
          <span key={crumb.href} className="flex items-center gap-1.5">
            ...
            {crumb.isLast ? (
              <span className="text-text-secondary text-[13px] font-medium">{crumb.label}</span>
            ) : (
